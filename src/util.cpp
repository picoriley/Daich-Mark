...
const char * const BITCOIN_CONF_FILENAME = "daichmark.conf";
const char * const BITCOIN_PID_FILENAME = "daichmark.pid";
...
#else
    const char* pszModule = "daichmark";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Daichmark";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Daichmark";
#else
    // Unix
    return pathRet / ".daichmark";
#endif
#endif
}
